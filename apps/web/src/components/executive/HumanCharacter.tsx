...
export function HumanCharacter({ position = [0, 0, 0], rotation = [0, 0, 0], scale = 1, suit, hair = '#1a1410', beard = false, glasses = false, bowtie = false, gesture = 'idle', name, status, seed = 0 }: HumanProps) {
  ...
  useEffect(() => {
    let id: number;
    const loop = () => { setBlink(0.1); window.setTimeout(() => setBlink(1), 110); id = window.setTimeout(loop, 2400 + Math.random() * 2600); };
    id = window.setTimeout(loop, 1000 + Math.random() * 1800);
